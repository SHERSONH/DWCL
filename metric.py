from sklearn.metrics import v_measure_score, adjusted_rand_score, accuracy_score, normalized_mutual_info_score, silhouette_score, calinski_harabasz_score
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=True, eval_z=True,
          times_for_K=1.0, test=True, hs_low = [], zs_low = []):
    ...
    sil_matrix_h = np.zeros(view)
    sil_matrix_z = np.zeros(view)
    ...
    if eval_h:
        ...
        for v in range(view):
            ...
            sil_matrix_h[v] = silhouette_score(h_vectors[v], y_pred.astype(float))
        ...
        print("z", np.array(z).shape) #(instance_num, n_view * dim)
        ...
        best_view = np.argmax(sil_matrix_h)
        for i in range(view):
            for j in range(view):
    ...
    if eval_z:
        ...
        for v in range(view):
            ...
            sil_matrix_z[v] = silhouette_score(z_vectors[v], y_pred.astype(float))
        ...
        nmi, ari, acc, pur = evaluate(labels_vector, pseudo_label)
        print('ACC = {:.4f} NMI = {:.4f} ARI = {:.4f} PUR = {:.4f}'.format(acc, nmi, ari, pur))
        best_view = np.argmax(sil_matrix_z)
    ...
    return acc, nmi, ari, pur, nmi_matrix_h, nmi_matrix_z, sil_matrix_h, sil_matrix_z
